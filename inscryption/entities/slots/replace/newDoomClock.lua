...
local POINTS = interp("{wavy freq=0.5 spacing=0.4 amp=0.5}{outline}Scale: %{colorEffect}%{points}{/c}/%{requiredPoints}")
...
local TEETH = interp("{wavy freq=0.6 spacing=0.8 amp=0.4}{outline}{c r=0.55 g=0.6 b=0.7}Teeths: %{teeth}")

umg.defineEntityType("inscryption:doom_clock", {
    image = "doom_clock",
    ...
    onDraw = function(ent, x,y, rot, sx,sy)
        ...
        local teeths = TEETH({
            teeth = math.floor(assert(lp.getMoney(ent)))
        })
        text.printRichCentered(teeths, font, x, y - 24, limit, "left", rot, sx*scale,sy*scale)
